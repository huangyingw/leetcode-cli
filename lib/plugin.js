...
Plugin.init = function(head) {
    ...
    for (let f of file.listCodeDir('lib/plugins')) {
        const p = f.data;
        if (!p) continue;
        log.info('found plugin: ' + p.name + '=' + p.ver);

        p.file = f.file;
        p.enabled = true;

        if (!(p.name in stats)) {
            log.info('p.builtin: ' + p.builtin);
            if (p.builtin) {
                log.info('new builtin plugin, enable by default');
                p.enabled = true;
            } else {
                log.info('new 3rd party plugin, disable by default');
                p.enabled = false;
            }
        }

        log.info('p.enabled: ' + p.enabled);
        installed.push(p);
    }
    ...
    for (let i = installed.length - 1; i >= 0; --i) {
        ...
        if (p.enabled) {
            p.init();
            log.info('inited plugin: ' + p.name);
        } else {
            log.info('skipped plugin: ' + p.name);
        }
    }
    ...
    for (let k of _.keys(stats)) {
        ...
        p.enabled = stats[k];
        missings.push(p);
        log.info('missing plugin:' + p.name);
    }
...
Plugin.install = function(name, cb) {
    const fullpath = file.pluginFile(name);
...
Plugin.installMissings = function(cb) {
    ...
    log.info('Installing missing plugins, might take a while ...');
